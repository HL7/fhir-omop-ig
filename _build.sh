...
if [[ $(uname -s) == 'Darwin' ]]; then
    echo "Don't run natively, use docker. This often requires many updates and installs."
    echo "Install and start Docker Desktop, then from within that shell, run:"
    echo "    docker run -it --rm -v .:/workspaces ghcr.io/bonfhir/ig-toolbox"
    echo " or " 
    echo "    docker run --rm -it -v $PWD/example/:/opt/ig-build-tools/workspace ghcr.io/miracum/ig-build-tools:latest"
    echo "Note: the prompt is odd: 3 workspaces ±|update_translate ✗|→  "
    exit 
fi
## brew install nvm
## nvm install 20
