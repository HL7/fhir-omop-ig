...
* value_as_number 0..1 decimal "Value as Number" "This is the numerical value of the Result of the Observation, if applicable and available. It is not expected that all Observations will have numeric results, rather, this field is here to house values should they exist."
...
* provider_id 0..1 integer "Provider" "The provider associated with the observation record, e.g. the provider who ordered the test or the provider who recorded the result."
* visit_occurrence_id 0..1 integer "Visit Occurence" "The visit during which the Observation occurred."
* visit_detail_id 0..1 integer "Visit Detail" "The VISIT_DETAIL record during which the Observation occurred. For example, if the Person was in the ICU at the time the VISIT_OCCURRENCE record would reflect the overall hospital stay and the VISIT_DETAIL record would reflect the ICU stay during the hospital visit."
...
//* observation_event_id 0..1 integer "Observation Event" 
