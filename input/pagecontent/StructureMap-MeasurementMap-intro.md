...
Implementation teams ought to note that FHIR's expressiveness often exceeds OMOP's structural capacity, requiring careful evaluation of value types that might need adaptation or exclusion if they cannot be effectively represented in the target OMOP model.
...
While UCUM provides a robust grammar-based approach for constructing unit expressions and supports machine-to-machine communication through consistent representation (using "mg" rather than variations like "milligram" or "milligramme"), implementation teams must exercise caution when working with UCUM units in clinical data warehouses. UCUM's flexibility in allowing the creation of new units through its grammar system, while beneficial for specialized domains, can introduce analytical challenges by generating separate groups of records with nuanced units if not properly managed. Care ought to be taken to normalize units of measure assigned to each result type within the datastore to support analytic consistency. For example, while UCUM might allow both "mg/dL" and "mg/100mL" as valid representations of the same conceptual unit, data normalization processes ought to require standardized unit assignment to ensure accurate cross-patient comparisons and longitudinal analysis.
