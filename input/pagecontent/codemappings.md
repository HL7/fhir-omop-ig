...
Implementers are strongly encouraged to capture sufficient detail during transformation — the source `system` URI, source `code`, the concepts returned by the terminology server, and the transformation decision that was made — so that anomalies can be reported to the Vocabulary Working Group with the evidence needed for timely review. This practice also supports the audit and quality-assurance documentation recommended elsewhere in this IG.
